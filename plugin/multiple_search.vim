...
command -nargs=1 MultipleSearch lua require'multiple_search'.matchadd({pattern = <q-args>})
command -nargs=? MultipleSearchDeleteGroup call luaeval("require'multiple_search'.matchdelete({group = (_A ~= '' and tonumber(_A) or nil)})", '<args>')
